...
RSpec.describe 'V1::OrdersController', type: :request do
  ...
  describe '#create' do
    ...
    it 'should not create order if user have another order within start date and end date' do
      order = create :order, starts_at: 2.days.since, expires_at: 5.day.since, user: @user
      ...
      expect{
        post '/v1/orders', params: { vehicle_id: @v1.id, starts_at: 1.days.since, expires_at: 6.days.since }, headers:  @user.create_new_auth_token
      }.to change(Order, :count).by 0
